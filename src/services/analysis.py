...
def get_rating_diff(df, color):
    return df[f'{color}_ratingDiff'].sum()

def get_opening_counts(df):
    return df['opening_eco'].value_counts(dropna=False)

def get_opening_counts_by_result(df, color):
    return df[df['winner'] == color]['opening_eco'].value_counts(dropna=False)

def get_rating_range(df, color):
    return df[f'{color}_rating'].min(), df[f'{color}_rating'].max()
